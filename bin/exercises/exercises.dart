library exercises;

import 'package:number_methods/input_data/exercises_transformed_data.dart';
import 'package:number_methods/methods_solving/gause_with_main_element_method.dart';
import 'package:number_methods/methods_solving/lu_decomposrition_method.dart';
import 'package:number_methods/methods_solving/llt_decomposrition_method.dart';
import 'package:number_methods/methods_solving/sweep_method.dart';
import 'package:number_methods/methods_solving/yacobi_method.dart';
import 'package:number_methods/methods_solving/zeidel_method.dart';

part 'all_numbers/exercise1.dart';
part 'all_numbers/exercise2.dart';
part 'all_numbers/exercise3.dart';
part 'all_numbers/exercise4.dart';
part 'all_numbers/exercise5.dart';
part 'all_numbers/exercise6.dart';
